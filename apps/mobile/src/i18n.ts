// Minimal local i18n for the worker vertical slice.
// Khmer-first. No external deps (i18next not installed in this slice).
// Strings are respectful adult copy; no glassmorphism / marketing fluff.
import { Locale } from './types';

type Dict = Record<string, string>;

// Khmer first, then English. Keys are stable English identifiers.
const km: Dict = {
  'app.name': 'SkillBridge',
  'app.uvp': 'រកការងារដែលអាចជឿទុកចិត្តបាន។ ផ្ទៀងផ្ទាត់ជំនាញរបស់អ្នក។ បង្កើតកំណត់ត្រាអាជីពដែលអ្នកជាម្ចាស់ផ្ទះ។',
  'welcome.title': 'ស្វាគមន៍មកកាន់ SkillBridge',
  'welcome.subtitle': 'រកការងារដែលបានផ្ទៀងផ្ទាត់ នៅកម្ពុជា',
  'welcome.chooseLanguage': 'ជ្រើសរើសភាសា',
  'welcome.khmer': 'ខ្មែរ',
  'welcome.english': 'English',
  'welcome.continue': 'ចាប់ផ្តើម',
  'welcome.demoNote': 'នេះគឺជាកម្មវិធីសាកល្បង។ គ្មានការភ្ជាប់ជាមួយម៉ាស៊ីនមេផលិតកម្មទេ។',

  'consent.title': 'ការសន្យាដែលយើងរក្សា',
  'consent.subtitle': 'មុនពេលអ្នកចាប់ផ្តើម — តើ SkillBridge ជាអ្វី និងមិនមែនជាអ្វី។',
  'consent.verify': 'យើងពិនិត្យក្រុមហ៊ុន និងការងារ — ប៉ុន្តែយើងមិនដែលនិយាយថា "សុវត្ថិភាព ១០០%" ទេ។',
  'consent.noPay': 'គ្មាននរណាម្នាក់អាចសុំឱ្យអ្នកបង់ប្រាក់ដើម្បីដាក់ពាក្យបានទេ។ សូមរាយការណ៍ ប្រសិនបើគេធ្វើបែបនេះ។',
  'consent.passport': 'លិខិតឆ្លងអាជីពរបស់អ្នកជារបស់អ្នក។ អ្នកគ្រប់គ្រងនូវអ្វីដែលត្រូវចែករំលែក។',
  'consent.report': 'របាយការណ៍មានលក្ខណៈសម្ងាត់ ហើយមិនបង្ហាញទៅអ្នកជ្រើសរើសទេ។',
  'consent.agree': 'ខ្ញុំយល់ — បន្ត',

  'tab.jobs': 'ការងារ',
  'tab.applications': 'ពាក្យសុំ',
  'tab.passport': 'លិខិតឆ្លង',
  'tab.help': 'ជំនួយ',
  'auth.wrongAccountTitle': 'គណនីនេះមិនមែនជាអ្នកកម្មករ',
  'auth.wrongAccountBody': 'កម្មវិធី SkillBridge សម្រាប់អ្នកកម្មករតម្រូវឱ្យចូលដោយគណនីអ្នកកម្មករ។ សូមចាកចេញ រួចចូលឡើងវិញជាមួយគណនីអ្នកកម្មករ។',
  'auth.signOut': 'ចាកចេញ',
  'auth.logout': 'ចាកចេញ',

  'phone.title': 'ចូលជាមួយលេខទូរស័ព្ទ',
  'phone.subtitle': 'យើងនឹងផ្ញើលេខកូដ ៦ ខ្ទង់ ដើម្បីផ្ទៀងផ្ទាត់លេខរបស់អ្នក។',
  'phone.label': 'លេខទូរស័ព្ទ',
  'phone.placeholder': 'ឧ. 012 345 678',
  'phone.continue': 'បន្ត',
  'phone.useEmail': 'ប្រើអ៊ីមែល និងពាក្យសម្ងាត់វិញ',
  'phone.invalid': 'សូមបញ្ចូលលេខទូរស័ព្ទឱ្យបានត្រឹមត្រូវ',
  'phone.smsNote': 'សាកល្បង៖ ការផ្ញើ SMS ត្រូវបានក្លែងបន្លំ — លេខកូដនឹងបង្ហាញខាងក្រោម។',
  'otp.title': 'បញ្ចូលលេខកូដ',
  'otp.subtitle': 'បានផ្ញើទៅ {phone}',
  'otp.placeholder': 'លេខកូដ ៦ ខ្ទង់',
  'otp.verify': 'ផ្ទៀងផ្ទាត់ និងបន្ត',
  'otp.resend': 'ផ្ញើកូដម្តងទៀត',
  'otp.back': 'ប្តូរលេខ',
  'otp.demoHint': 'សាកល្បង៖ លេខកូដរបស់អ្នកគឺ {code}',
  'otp.invalid': 'សូមបញ្ចូលលេខកូដ ៦ ខ្ទង់',
  'common.error': 'កំហុស',

  'jobs.title': 'ការងារដែលបានផ្ទៀងផ្ទាត់',
  'jobs.subtitle': 'ការងាររោងចក្រដែលបានពិនិត្យរួច',
  'jobs.search': 'ស្វែងរកការងារ ឬជំនាញ',
  'jobs.filterNear': 'ក្បែរខ្ញុំ',
  'jobs.filterDay': 'វេនថ្ងៃ',
  'jobs.filterSalary': 'ប្រាក់ខែ',
  'jobs.jobChecked': 'ការងារបានផ្ទៀងផ្ទាត់',
  'jobs.companyChecked': 'ក្រុមហ៊ុនបានផ្ទៀងផ្ទាត់',
  'jobs.identityChecked': 'អត្តសញ្ញាណបានផ្ទៀងផ្ទាត់',
  'jobs.notVerified': 'មិនទាន់បានផ្ទៀងផ្ទាត់',
  'jobs.youMatch': 'អ្នកផ្គូផ្គង {n}/{m} ជំនាញ',
  'jobs.km': 'គម',
  'jobs.viewJob': 'មើលការងារ',
  'jobs.results': 'ការងារ {n} បានផ្ទៀងផ្ទាត់',
  'jobs.loading': 'កំពុងផ្ទុកការងារ...',
  'jobs.empty': 'មិនមានការងារដែលត្រូវនឹងតម្រងទេ។',
  'jobs.emptySub': 'សាកល្បងលុបតម្រង ឬជ្រើស "ក្បែរខ្ញុំ"។',
  'jobs.error': 'មិនអាចផ្ទុកការងារបានទេ។',
  'jobs.retry': 'ព្យាយាមម្តងទៀត',

  'job.pay': 'ប្រាក់ខែ',
  'job.shift': 'វេន',
  'job.distance': 'ចម្ងាយ',
  'job.day': 'វេនថ្ងៃ',
  'job.night': 'វេនយប់',
  'job.rotating': 'វេនស្លែង',
  'job.flexible': 'បត់បែន',
  'job.fullTime': 'ពេញម៉ោង',
  'job.type': 'ប្រភេទការងារ',
  'job.contract': 'កិច្ចសន្យា',
  'job.seasonal': 'រដូវកាល',
  'job.lastChecked': 'បានពិនិត្យ {date}',
  'job.seeWhatChecked': 'មើលអ្វីដែលបានផ្ទៀងផ្ទាត់',
  'job.whyMatches': 'ហេតុអ្វីវាផ្គូផ្គងអ្នក',
  'job.matchingSkills': 'ជំនាញដែលផ្គូផ្គង',
  'job.missingRequirements': 'តម្រូវការខ្វះខាត',
  'job.conditions': 'លក្ខខណ្ឌការងារ',
  'job.accommodation': 'កន្លែងស្នាក់នៅ',
  'job.transport': 'ឡានដឹកជញ្ជូន',
  'job.overtime': 'បន្ថែមម៉ោង',
  'job.leave': 'ច្បាប់',
  'job.weChecked': 'យើងបានពិនិត្យក្រុមហ៊ុន និងការបើកតួនេះ',
  'job.whatChecked': 'អ្វីដែលបានផ្ទៀងផ្ទាត់',
  'job.cannotGuarantee': 'អ្វីដែល SkillBridge មិនអាចធានាបាន',
  'job.applyWithPassport': 'ដាក់ពាក្យជាមួយលិខិតឆ្លង',
  'job.reportConcern': 'រាយការណ៍កង្វល់',
  'job.back': 'ត្រឡប់',
  'job.details': 'ព័ត៌មានការងារ',
  'job.noChecksListed': 'មិនមានអ្វីដែលបានផ្ទៀងផ្ទាត់ត្រូវបានរាយទេ។',
  'job.cannotGuaranteeEmpty': 'យើងមិនអាចធានាអត្តិបត្តិរបស់ថៅកែបានទេ។',
  'job.noSkillsListed': 'មិនមានជំនាញដែលផ្គូផ្គងទេ។',
  'passport.noWorkRecords': 'មិនទាន់មានកំណត់ត្រាការងារដែលផ្ទៀងផ្ទាត់។',
  'passport.noSkills': 'មិនទាន់មានជំនាញត្រូវបានកត់ត្រា។',
  'passport.noLanguages': 'មិនទាន់មានភាសាត្រូវបានកត់ត្រា។',
  'passport.noSafety': 'មិនទាន់មានវិញ្ញាបនបត្រសុវត្ថិភាពត្រូវបានកត់ត្រា។',
  'passport.identityNotVerified': 'អត្តសញ្ញាណមិនបានផ្ទៀងផ្ទាត់',

  'apply.review': 'ពិនិត្យពាក្យសុំ',
  'apply.companyReceives': 'ក្រុមហ៊ុននឹងទទួលបាន៖',
  'apply.nameContact': 'ឈ្មោះ និងទំនាក់ទំនក',
  'apply.matchingSkills': 'ជំនាញដែលផ្គូផ្គង',
  'apply.workRecords': 'កំណត់ត្រាការងារដែលផ្ទៀងផ្ទាត់',
  'apply.hideCertificate': 'លាក់ព័ត៌មានវិញ្ញាបនបត្រ',
  'apply.certHidden': 'ព័ត៌មានជំនាញ និងកំណត់ត្រាការងារត្រូវបានលាក់ពីក្រុមហ៊ុន។',
  'apply.noPayment': 'មិនតម្រូវឱ្យបង់ប្រាក់ដើម្បីដាក់ពាក្យទេ។',
  'apply.demoNote': 'នេះគឺជាសាកល្បង។ ពាក្យសុំមិនត្រូវបានផ្ញើទៅក្រុមហ៊ុនពិតទេ។',
  'apply.submit': 'ដាក់ពាក្យសុំ',
  'apply.submitting': 'កំពុងដាក់ពាក្យ...',
  'apply.done': 'ពាក្យសុំបានដាក់',
  'apply.doneSub': 'អ្នកអាចតាមដានស្ថានភាពនៅផ្ទាំង "ពាក្យសុំ"។',
  'apply.viewApplications': 'មើលពាក្យសុំរបស់ខ្ញុំ',
  'apply.alreadyApplied': 'អ្នកបានដាក់ពាក្យសុំការងារនេះរួចហើយ។',

  'applications.title': 'ពាក្យសុំរបស់ខ្ញុំ',
  'applications.empty': 'អ្នកមិនទាន់ដាក់ពាក្យសុំទេ។',
  'applications.emptySub': 'ស្វែងរកការងារ ហើយដាក់ពាក្យជាមួយលិខិតឆ្លងរបស់អ្នក។',
  'applications.submitted': 'បានដាក់ពាក្យ',
  'applications.underReview': 'កំពុងពិនិត្យ',
  'applications.interview': 'សម្ភាសន៍',
  'applications.accepted': 'បានទទួលយក',
  'applications.declined': 'បានបដិសេធ',
  'applications.withdrawn': 'បានដកពាក្យ',
  'applications.submittedAt': 'បានដាក់ {date}',
  'applications.viewJob': 'មើលការងារ',
  'applications.demo': 'សាកល្បង',

  'notifications.title': 'ការជូនដំណឹង',
  'notifications.empty': 'មិនទាន់មានការជូនដំណឹងទេ។',
  'notifications.emptySub': 'ស្ថានភាពពាក្យសុំ និងការផ្ទៀងផ្ទាត់របស់អ្នកនឹងបង្ហាញនៅទីនេះ។',
  'notifications.applicationStatus': 'ស្ថានភាពពាក្យសុំ',

  'passport.title': 'លិខិតឆ្លងអាជីព',
  'passport.share': 'ចែករំលែក',
  'passport.identity': 'អត្តសញ្ញាណ',
  'passport.identityVerified': 'អត្តសញ្ញាណបានផ្ទៀងផ្ទាត់',
  'passport.verifiedWork': 'ការងារដែលផ្ទៀងផ្ទាត់',
  'passport.skills': 'ជំនាញ',
  'passport.verified': 'បានផ្ទៀងផ្ទាត់',
  'passport.selfDeclared': 'ប្រកាសដោយខ្លួនឯង',
  'passport.verificationRequested': 'កំពុងស្នើសុំផ្ទៀងផ្ទាត់',
  'passport.requestVerification': 'ស្នើសុំការផ្ទៀងផ្ទាត់',
  'passport.deleteRecord': 'លុប',
  'passport.addRecord': 'បន្ថែមការងារមុន',
  'passport.workRecordsDemo': 'សាកល្បង៖ ការបន្ថែមការងារ និងស្នើសុំផ្ទៀងផ្ទាត់ ត្រូវការការភ្ជាប់ API។',

  'workrecord.title': 'បន្ថែមការងារមុន',
  'workrecord.role': 'តួនាទី',
  'workrecord.rolePlaceholder': 'ឧ. អ្នកប្រតិបត្តិការម៉ាស៊ីនដេរ',
  'workrecord.company': 'ក្រុមហ៊ុន',
  'workrecord.companyPlaceholder': 'ឧ. រោងចក្រកាត់ដេរ ABC',
  'workrecord.workplace': 'កន្លែងធ្វើការ (ស្រេចចិត្ត)',
  'workrecord.start': 'ឆ្នាំចាប់ផ្តើម (YYYY-MM)',
  'workrecord.end': 'ឆ្នាំបញ្ចប់ (YYYY-MM, ទុកចន្លោះបើនៅតែធ្វើការ)',
  'workrecord.save': 'រក្សាទុក',
  'workrecord.required': 'សូមបំពេញតួនាទី ក្រុមហ៊ុន និងឆ្នាំចាប់ផ្តើម',
  'workrecord.saved': 'បានបន្ថែមការងារ',
  'workrecord.requested': 'បានផ្ញើសំណើផ្ទៀងផ្ទាត់',
  'workrecord.deleted': 'បានលុបកំណត់ត្រា',
  'workrecord.verifiedLocked': 'កំណត់ត្រាដែលបានផ្ទៀងផ្ទាត់មិនអាចលុបបានទេ។',

  'onboarding.title': 'បង្កើតលិខិតឆ្លងអាជីពរបស់អ្នក',
  'onboarding.subtitle': 'ជ្រើសរើសចំណូលចិត្ត ដើម្បីឱ្យយើងអាចផ្គូផ្គងការងារដែលសមនឹងអ្នក។',
  'onboarding.area': 'តំបន់ដែលចង់ធ្វើការ',
  'onboarding.skills': 'ជំនាញរបស់អ្នក',
  'onboarding.availability': 'វេនការងារ',
  'onboarding.languages': 'ភាសា',
  'onboarding.save': 'រក្សាទុក និងបន្ត',
  'onboarding.saving': 'កំពុងរក្សាទុក...',
  'onboarding.required': 'សូមជ្រើសរើសតំបន់ និងវេនការងារ',
  'onboarding.saveError': 'មិនអាចរក្សាទុកបានទេ។ សូមព្យាយាមម្តងទៀត។',
  'onboarding.area.phnomPenh': 'ភ្នំពេញ',
  'onboarding.area.kandal': 'កណ្តាល',
  'onboarding.area.kampongSpeu': 'កំពង់ស្ពឺ',
  'onboarding.area.sihanoukville': 'ព្រះសីហនុ',
  'onboarding.skill.sewing': 'ដេរ',
  'onboarding.skill.qualityControl': 'ត្រួតពិនិត្យគុណភាព',
  'onboarding.skill.cutting': 'កាត់ក្រណាត់',
  'onboarding.skill.packing': 'វេចខ្ចប់',
  'onboarding.skill.warehouse': 'ឃ្លាំង',
  'onboarding.skill.forklift': 'បើករថយន្តលើក',
  'onboarding.skill.machineOperator': 'ប្រតិបត្តិការម៉ាស៊ីន',
  'passport.languages': 'ភាសា',
  'passport.safety': 'វិញ្ញាបនបត្រសុវត្ថិភាព',
  'passport.shareOn': 'ការចែករំលែកបើក',
  'passport.shareOff': 'ការចែករំលែកបិទ',
  'passport.shareDemo': 'សាកល្បង៖ ការចែករំលែកត្រូវបានក្លែងបន្លំ។ លិខិតឆ្លងរបស់អ្នកមិនត្រូវបានផ្ញើទៅទីណាទេ។',
  'passport.shareCreate': 'បង្កើតតំណចែករំលែក',
  'passport.shareExpires': 'ផុតកំណត់ {date}',
  'passport.revoke': 'ដកហូត',
  'passport.revokeConfirm': 'តើអ្នកប្រាកដថាចង់ដកហូតតំណនេះទេ? អ្នកដែលមានតំណនឹងលែងឃើញលិខិតឆ្លងរបស់អ្នក។',
  'passport.shareEmpty': 'មិនទាន់មានតំណសកម្មទេ។',
  'passport.shareReal': 'តំណនេះពិតប្រាកដ — អ្នកជ្រើសរើសអាចបើកវាដើម្បីមើលលិខិតឆ្លងរបស់អ្នក។ វាផុតកំណត់ក្នុងរយៈពេល ២៤ ម៉ោង។',
  'passport.demoState': 'ស្ថានភាពសាកល្បង',
  'passport.demoNote': 'លិខិតឆ្លងនេះស្ថិតនៅក្នុងឧបករណ៍របស់អ្នក។ វាមិនត្រូវបានផ្ញើទៅម៉ាស៊ីនមេទេ។',
  'passport.readiness': 'រួចរាល់ {n}% សម្រាប់ពាក្យសុំ',
  'passport.edit': 'កែសម្រួល',
  'passport.editTitle': 'កែសម្រួលលិខិតឆ្លង',
  'passport.editSubtitle': 'ព័ត៌មានដែលអ្នកបញ្ចូលនឹងផ្ទៀងផ្ទាត់ដោយខ្លួនឯង។ អត្តសញ្ញាណ និងកំណត់ត្រាការងារដែលផ្ទៀងផ្ទាត់ ត្រូវគ្រប់គ្រងដោយអ្នកផ្ទៀងផ្ទាត់។',
  'profile.fullName': 'ឈ្មោះពេញ',
  'profile.preferredArea': 'តំបន់ដែលចង់ធ្វើការ',
  'profile.availability': 'ពេលវេលាអាចធ្វើការបាន',
  'profile.skills': 'ជំនាញ',
  'profile.languages': 'ភាសា',
  'profile.addOne': 'បន្ថែម...',
  'profile.save': 'រក្សាទុក',
  'profile.saving': 'កំពុងរក្សាទុក...',
  'profile.saved': 'បានរក្សាទុករួចរាល់',
  'profile.saveError': 'មិនអាចរក្សាទុកបានទេ។ សូមព្យាយាមម្តងទៀត។',
  'profile.fullNameHint': 'ឈ្មោះពិតរបស់អ្នក ដូចដែលអ្នកចង់បង្ហាញដល់ថៅកែ។',
  'profile.availabilityHint': 'ឧ. ពេលព្រឹក ឬអាចធ្វើការបានគ្រប់ពេល',

  'help.title': 'សុវត្ថិភាព និងជំនួយ',
  'help.subtitle': 'ការពារខ្លួនពីការក្លែងបន្លំ',
  'help.scamTitle': 'សញ្ញាប្រុងប្រយ័ត្នក្លែងបន្លំ',
  'help.cat.payment': 'គេសុំឱ្យអ្នកបង់ប្រាក់',
  'help.cat.false': 'ព័ត៌មានការងារមិនពិត',
  'help.cat.identity': 'ការសង្ស័យអត្តសញ្ញាណអ្នកជ្រើសរើស',
  'help.cat.unsafe': 'ការទំនាក់ទំនងមិនមានសុវត្ថិភាព',
  'help.cat.other': 'រឿងផ្សេងទៀត',
  'help.report': 'រាយការណ៍កង្វល់',
  'help.noApplyNeeded': 'អ្នកអាចរាយការណ៍ដោយមិនចាំបាច់ដាក់ពាក្យ។',
  'help.promise': 'SkillBridge មិនដែលនិយាយថា "សុវត្ថិភាព ១០០%"។ យើងផ្ទៀងផ្ទាត់ ប៉ុន្តែមិនអាចធានាអត្តិបត្តិរបស់ថៅកែបានទេ។',
  'safety.title': 'សុវត្ថិភាពរបស់ខ្ញុំ',
  'safety.entry': 'របាយការណ៍ និងការទប់ស្កាត់របស់ខ្ញុំ',
  'safety.myReports': 'របាយការណ៍របស់ខ្ញុំ',
  'safety.noReports': 'មិនទាន់មានរបាយការណ៍ទេ',
  'safety.noReportsSub': 'អ្នកអាចរាយការណ៍កង្វល់ដោយមិនចាំបាច់ដាក់ពាក្យ។',
  'safety.blockedJobs': 'ការងារដែលបានទប់ស្កាត់',
  'safety.noBlocked': 'គ្មានការងារដែលបានទប់ស្កាត់ទេ',
  'safety.noBlockedSub': 'ការងារដែលអ្នកទប់ស្កាត់នឹងលាក់ពីបញ្ជីការងារ។',
  'safety.unblock': 'ដោះទប់ស្កាត់',
  'safety.loadError': 'មិនអាចផ្ទុកទិន្នន័យសុវត្ថិភាពរបស់អ្នកបានទេ។',
  'common.retry': 'ព្យាយាមម្តងទៀត',
  'job.blockJob': 'ទប់ស្កាត់ការងារនេះ',
  'job.blockConfirm': 'ទប់ស្កាត់ការងារនេះ? វានឹងលាក់ពីបញ្ជីការងាររបស់អ្នក។',
  'job.blocked': 'បានទប់ស្កាត់',
  'job.blockedSub': 'ការងារនេះត្រូវបានលាក់ពីបញ្ជីរបស់អ្នក។ គ្រប់គ្រងនៅក្រោម Help → សុវត្ថិភាពរបស់ខ្ញុំ។',

  'report.title': 'រាយការណ៍កង្វល់',
  'report.whatHappened': 'តើមានរឿងអ្វីកើតឡើង?',
  'report.whatPlaceholder': 'ឧ. គេសុំឱ្យខ្ញុំបង់ប្រាក់ ២០ដុល្លារមុនពេលសម្ភាសន៍។',
  'report.addEvidence': 'បន្ថែមភ័ស្តុតាង (ស្រេចចិត្ត)',
  'report.evidencePlaceholder': 'តំណភ្ជាប់ ឬព័ត៌មានបន្ថែម...',
  'report.send': 'ផ្ញើរដាក់រាយការណ៍',
  'report.notShown': 'របាយការណ៍មិនត្រូវបង្ហាញទៅអ្នកជ្រើសរើសទេ។',
  'report.demo': 'នេះគឺជាសាកល្បង។ របាយការណ៍មិនត្រូវបានផ្ញើទៅទីណាទេ។',
  'report.confirmTitle': 'បានកត់ត្រារបាយការណ៍សាកល្បង',
  'report.confirmSub': 'អ្នកជ្រើសរើសមិនឃើញរបាយការណ៍នេះទេ។ នេះគឺជាកម្មវិធីសាកល្បង — របាយការណ៍មិនត្រូវបានផ្ញើទៅម៉ាស៊ីនមេទេ។',
  'report.done': 'រួចរាល់',
  'report.status.submitted': 'បានដាក់',
  'report.status.under_review': 'កំពុងពិនិត្យ',
  'report.status.resolved': 'បានដោះស្រាយ',
  'report.trackStatus': 'អ្នកអាចតាមដានស្ថានភាពរបាយការណ៍នៅទីនេះ។',
  'report.confirmSubReal': 'អ្នកជ្រើសរើសមិនឃើញរបាយការណ៍នេះទេ។ យើងនឹងជូនដំណឹងពេលមានការធ្វើបច្ចុប្បន្នភាព។',

  'common.demo': 'សាកល្បង',
  'common.yes': 'មាន',
  'common.no': 'គ្មាន',
  'common.close': 'បិទ',
  'common.back': 'ត្រឡប់',
  'common.cancel': 'បោះបង់',
  'common.km': 'ខ្មែរ',
  'common.en': 'EN',
  'common.loading': 'កំពុងផ្ទុក...',
};

const en: Dict = {
  'app.name': 'SkillBridge',
  'app.uvp': 'Find trusted work. Prove your skills. Build a career record you own.',
  'welcome.title': 'Welcome to SkillBridge',
  'welcome.subtitle': 'Verified work, in Cambodia',
  'welcome.chooseLanguage': 'Choose your language',
  'welcome.khmer': 'ខ្មែរ',
  'welcome.english': 'English',
  'welcome.continue': 'Get started',
  'welcome.demoNote': 'This is a demo prototype. It is not connected to a live server.',

  'consent.title': 'A promise we keep',
  'consent.subtitle': 'Before you begin — what SkillBridge is, and is not.',
  'consent.verify': 'We check companies and jobs — but we never say "100% safe".',
  'consent.noPay': 'No one may ask you to pay to apply. Report it if they do.',
  'consent.passport': 'Your Passport belongs to you. You control what is shared.',
  'consent.report': 'Reports are confidential and never shown to the recruiter.',
  'consent.agree': 'I understand — continue',

  'tab.jobs': 'Jobs',
  'tab.applications': 'Applications',
  'tab.passport': 'Passport',
  'tab.help': 'Help',
  'auth.wrongAccountTitle': 'This is not a worker account',
  'auth.wrongAccountBody': 'The SkillBridge worker app requires a worker account. Please sign out and log in with a worker account.',
  'auth.signOut': 'Sign out',
  'auth.logout': 'Log out',

  'phone.title': 'Sign in with your phone',
  'phone.subtitle': 'We will send a 6-digit code to verify your number.',
  'phone.label': 'Phone number',
  'phone.placeholder': 'e.g. 012 345 678',
  'phone.continue': 'Continue',
  'phone.useEmail': 'Use email & password instead',
  'phone.invalid': 'Enter a valid phone number',
  'phone.smsNote': 'Demo: SMS delivery is simulated — the code appears below.',
  'otp.title': 'Enter the code',
  'otp.subtitle': 'Sent to {phone}',
  'otp.placeholder': '6-digit code',
  'otp.verify': 'Verify & continue',
  'otp.resend': 'Resend code',
  'otp.back': 'Change number',
  'otp.demoHint': 'Demo: your code is {code}',
  'otp.invalid': 'Enter the 6-digit code',
  'common.error': 'Error',

  'jobs.title': 'Verified jobs',
  'jobs.subtitle': 'Reviewed factory jobs only',
  'jobs.search': 'Search job or skill',
  'jobs.filterNear': 'Near me',
  'jobs.filterDay': 'Day shift',
  'jobs.filterSalary': 'Salary',
  'jobs.jobChecked': 'Job checked',
  'jobs.companyChecked': 'Company checked',
  'jobs.identityChecked': 'Identity checked',
  'jobs.notVerified': 'Not verified',
  'jobs.youMatch': 'You match {n} of {m} skills',
  'jobs.km': 'km',
  'jobs.viewJob': 'View job',
  'jobs.results': '{n} verified jobs',
  'jobs.loading': 'Loading jobs...',
  'jobs.empty': 'No jobs match your filters.',
  'jobs.emptySub': 'Try clearing filters or selecting "Near me".',
  'jobs.error': 'Could not load jobs.',
  'jobs.retry': 'Retry',

  'job.pay': 'Pay',
  'job.shift': 'Shift',
  'job.distance': 'Distance',
  'job.day': 'Day',
  'job.night': 'Night',
  'job.rotating': 'Rotating',
  'job.flexible': 'Flexible',
  'job.fullTime': 'Full-time',
  'job.type': 'Job type',
  'job.contract': 'Contract',
  'job.seasonal': 'Seasonal',
  'job.lastChecked': 'Checked {date}',
  'job.seeWhatChecked': 'See what was checked',
  'job.whyMatches': 'Why it matches you',
  'job.matchingSkills': 'Matching skills',
  'job.missingRequirements': 'Missing requirements',
  'job.conditions': 'Work conditions',
  'job.accommodation': 'Accommodation',
  'job.transport': 'Transport',
  'job.overtime': 'Overtime',
  'job.leave': 'Leave',
  'job.weChecked': 'We checked the company and this vacancy',
  'job.whatChecked': 'What was checked',
  'job.cannotGuarantee': "What SkillBridge cannot guarantee",
  'job.applyWithPassport': 'Apply with Passport',
  'job.reportConcern': 'Report a concern',
  'job.back': 'Back',
  'job.details': 'Job details',
  'job.noChecksListed': 'No verification checks are listed yet.',
  'job.cannotGuaranteeEmpty': "We can't guarantee employer conduct — only what we verify.",
  'job.noSkillsListed': 'No matching skills listed yet.',
  'job.cannotGuaranteeHeader': "What SkillBridge cannot guarantee",
  'passport.noWorkRecords': 'No verified work records yet.',
  'passport.noSkills': 'No skills recorded yet.',
  'passport.noLanguages': 'No languages recorded yet.',
  'passport.noSafety': 'No safety qualifications recorded yet.',
  'passport.identityNotVerified': 'Identity not verified',

  'apply.review': 'Review application',
  'apply.companyReceives': 'The company will receive:',
  'apply.nameContact': 'Name and contact',
  'apply.matchingSkills': 'Matching skills',
  'apply.workRecords': 'Verified work records',
  'apply.hideCertificate': 'Hide certificate details',
  'apply.certHidden': 'Your skills and work records are hidden from the company.',
  'apply.noPayment': 'No payment is required to apply.',
  'apply.demoNote': 'This is a demo. Your application is not sent to a real company.',
  'apply.submit': 'Submit application',
  'apply.submitting': 'Submitting...',
  'apply.done': 'Application submitted',
  'apply.doneSub': 'Track status in the Applications tab.',
  'apply.viewApplications': 'View my applications',
  'apply.alreadyApplied': "You've already applied to this job.",

  'applications.title': 'My applications',
  'applications.empty': 'You have not applied yet.',
  'applications.emptySub': 'Browse jobs and apply with your Passport.',
  'applications.submitted': 'Submitted',
  'applications.underReview': 'Under review',
  'applications.interview': 'Interview',
  'applications.accepted': 'Accepted',
  'applications.declined': 'Declined',
  'applications.withdrawn': 'Withdrawn',
  'applications.submittedAt': 'Submitted {date}',
  'applications.viewJob': 'View job',
  'applications.demo': 'Demo',

  'notifications.title': 'Notifications',
  'notifications.empty': 'No notifications yet.',
  'notifications.emptySub': 'Your application status and verification updates will appear here.',
  'notifications.applicationStatus': 'Application status',

  'passport.title': 'Career Passport',
  'passport.share': 'Share',
  'passport.identity': 'Identity',
  'passport.identityVerified': 'Identity verified',
  'passport.verifiedWork': 'Verified work',
  'passport.skills': 'Skills',
  'passport.verified': 'Verified',
  'passport.verificationRequested': 'Verification requested',
  'passport.deleteRecord': 'Delete',
  'passport.addRecord': 'Add past job',
  'passport.workRecordsDemo': 'Demo: adding jobs and requesting verification requires the API.',

  'workrecord.title': 'Add past job',
  'workrecord.role': 'Role',
  'workrecord.rolePlaceholder': 'e.g. Sewing machine operator',
  'workrecord.company': 'Company',
  'workrecord.companyPlaceholder': 'e.g. ABC Garment Factory',
  'workrecord.workplace': 'Workplace (optional)',
  'workrecord.start': 'Start (YYYY-MM)',
  'workrecord.end': 'End (YYYY-MM, leave blank if current)',
  'workrecord.save': 'Save',
  'workrecord.required': 'Please fill in role, company, and start date',
  'workrecord.saved': 'Work record added',
  'workrecord.requested': 'Verification request sent',
  'workrecord.deleted': 'Record deleted',
  'workrecord.verifiedLocked': 'Verified records cannot be deleted.',

  'onboarding.title': 'Set up your Passport',
  'onboarding.subtitle': 'Choose your preferences so we can match jobs that fit you.',
  'onboarding.area': 'Where you want to work',
  'onboarding.skills': 'Your skills',
  'onboarding.availability': 'Work shift',
  'onboarding.languages': 'Languages',
  'onboarding.save': 'Save & continue',
  'onboarding.saving': 'Saving...',
  'onboarding.required': 'Please choose an area and a shift',
  'onboarding.saveError': 'Could not save. Please try again.',
  'onboarding.area.phnomPenh': 'Phnom Penh',
  'onboarding.area.kandal': 'Kandal',
  'onboarding.area.kampongSpeu': 'Kampong Speu',
  'onboarding.area.sihanoukville': 'Sihanoukville',
  'onboarding.skill.sewing': 'Sewing',
  'onboarding.skill.qualityControl': 'Quality control',
  'onboarding.skill.cutting': 'Cutting',
  'onboarding.skill.packing': 'Packing',
  'onboarding.skill.warehouse': 'Warehouse',
  'onboarding.skill.forklift': 'Forklift',
  'onboarding.skill.machineOperator': 'Machine operator',
  'passport.selfDeclared': 'Self-declared',
  'passport.languages': 'Languages',
  'passport.safety': 'Safety qualifications',
  'passport.shareOn': 'Sharing on',
  'passport.shareOff': 'Sharing off',
  'passport.shareDemo': 'Demo: sharing is simulated. Your Passport is not sent anywhere.',
  'passport.shareCreate': 'Create a share link',
  'passport.shareExpires': 'Expires {date}',
  'passport.revoke': 'Revoke',
  'passport.revokeConfirm': 'Revoke this link? Anyone with it will no longer see your Passport.',
  'passport.shareEmpty': 'No active links yet.',
  'passport.shareReal': 'This link is real — a recruiter can open it to see your Passport. It expires in 24 hours.',
  'passport.requestVerification': 'Request work verification',
  'passport.demoState': 'Demo state',
  'passport.demoNote': 'This Passport stays on your device. It is not sent to a server.',
  'passport.readiness': 'Ready for applications: {n}%',
  'passport.edit': 'Edit',
  'passport.editTitle': 'Edit Passport',
  'passport.editSubtitle': 'Fields you enter are self-declared. Identity and verified work records are controlled by verifiers.',
  'profile.fullName': 'Full name',
  'profile.preferredArea': 'Preferred work area',
  'profile.availability': 'Availability',
  'profile.skills': 'Skills',
  'profile.languages': 'Languages',
  'profile.addOne': 'Add...',
  'profile.save': 'Save',
  'profile.saving': 'Saving...',
  'profile.saved': 'Saved',
  'profile.saveError': "Couldn't save. Please try again.",
  'profile.fullNameHint': 'Your real name, as you would show it to an employer.',
  'profile.availabilityHint': 'e.g. Mornings, or available any time',

  'help.title': 'Safety & help',
  'help.subtitle': 'Protect yourself from scams',
  'help.scamTitle': 'Scam warning signs',
  'help.cat.payment': 'Asked me to pay money',
  'help.cat.false': 'Job information is false',
  'help.cat.identity': 'Recruiter identity concern',
  'help.cat.unsafe': 'Unsafe or abusive contact',
  'help.cat.other': 'Something else',
  'help.report': 'Report a concern',
  'help.noApplyNeeded': 'You can report without applying.',
  'help.promise': 'SkillBridge never says "100% safe". We verify, but cannot guarantee employer conduct.',
  'safety.title': 'My safety',
  'safety.entry': 'My reports & blocked jobs',
  'safety.myReports': 'My reports',
  'safety.noReports': 'No reports yet',
  'safety.noReportsSub': 'You can report a concern without applying.',
  'safety.blockedJobs': 'Blocked jobs',
  'safety.noBlocked': 'No blocked jobs',
  'safety.noBlockedSub': 'Jobs you block are hidden from your feed.',
  'safety.unblock': 'Unblock',
  'safety.loadError': 'Could not load your safety data.',
  'common.retry': 'Retry',
  'job.blockJob': 'Block this job',
  'job.blockConfirm': 'Block this job? It will be hidden from your job list.',
  'job.blocked': 'Blocked',
  'job.blockedSub': 'This job is hidden from your list. Manage it under Help → My safety.',

  'report.title': 'Report a concern',
  'report.whatHappened': 'What happened?',
  'report.whatPlaceholder': 'e.g. They asked me to pay $20 before the interview.',
  'report.addEvidence': 'Add optional evidence',
  'report.evidencePlaceholder': 'A link or extra details...',
  'report.send': 'Send report',
  'report.notShown': 'Reports are not shown to the recruiter.',
  'report.demo': 'This is a demo. Your report is not sent anywhere.',
  'report.confirmTitle': 'Demo report recorded',
  'report.confirmSub': 'The recruiter does not see this report. This is a local prototype — the report is not sent to a server.',
  'report.done': 'Done',
  'report.status.submitted': 'Submitted',
  'report.status.under_review': 'Under review',
  'report.status.resolved': 'Resolved',
  'report.trackStatus': 'Track this report’s status here.',
  'report.confirmSubReal': 'The recruiter does not see this report. We will notify you when there is an update.',

  'common.demo': 'Demo',
  'common.yes': 'Yes',
  'common.no': 'No',
  'common.close': 'Close',
  'common.back': 'Back',
  'common.cancel': 'Cancel',
  'common.km': 'ខ្មែរ',
  'common.en': 'EN',
  'common.loading': 'Loading...',
};

const TABLES: Record<Locale, Dict> = { km, en };

let current: Locale = 'km';

export function getLocale(): Locale {
  return current;
}

export function setLocale(locale: Locale): void {
  current = locale;
}

export function t(
  key: string,
  vars?: Record<string, string | number>,
  localeOverride?: Locale
): string {
  const locale = localeOverride ?? current;
  const table = TABLES[locale] ?? en;
  let str = table[key] ?? en[key] ?? key;
  if (vars) {
    for (const [k, v] of Object.entries(vars)) {
      str = str.replace(new RegExp(`\\{${k}\\}`, 'g'), String(v));
    }
  }
  return str;
}

export function formatDate(iso: string, locale: Locale = current): string {
  // Demo date formatting. Khmer uses Khmer month names + Khmer numerals;
  // English uses short Latin month names. No external locale dependency.
  const d = new Date(iso);
  if (isNaN(d.getTime())) return iso;
  const day = d.getDate();
  const month = d.getMonth();
  const year = d.getFullYear();

  if (locale === 'km') {
    const KM_MONTHS = [
      'មករា', 'កុម្ភៈ', 'មីនា', 'មេសា', 'ឧសភា', 'មិថុនា',
      'កក្កដា', 'សីហា', 'កញ្ញា', 'តុលា', 'វិច្ឆិកា', 'ធ្នូ',
    ];
    const KM_DIGITS = ['០', '១', '២', '៣', '៤', '៥', '៦', '៧', '៨', '៩'];
    const toKmNum = (n: number) => String(n).split('').map((c) => KM_DIGITS[Number(c)]).join('');
    return `${toKmNum(day)} ${KM_MONTHS[month]} ${toKmNum(year)}`;
  }

  const enMonth = d.toLocaleString('en-US', { month: 'short' });
  return `${day} ${enMonth} ${year}`;
}
